namespace LM.ImageComments.EditorComponent
{
	...
	public class ImageAdornmentManager : ITagger<ErrorTag>, IDisposable
	{
		...
		public static int EditIterator = 0;
		...
		internal void InsertImageLinkAt(string uri, VirtualSnapshotPoint location)
		{
			try
			{
				uri = _variableExpander.SubstituteProjectOrSolutionDir(uri);
			} catch (Exception e)
			{
				Debug.WriteLine(e.Message);
			}

			ITextViewLine line = _view.TextViewLines.GetTextViewLineContainingBufferPosition(location.Position);

			int positionInLine = location.Position.Position - line.Start;
			string lineText = line.Extent.GetText();
			int imageCommentPosition = ImageCommentParser.MatchComment(_contentTypeName, lineText);

			//_view.TextBuffer.
			ITextEdit edit = _view.TextBuffer.CreateEdit(EditOptions.None, EditIterator++, null);
			string imageLink = "<image url=\"" + uri + "\"/>";
			string comment = ImageCommentParser.GetLineCommentStart(_contentTypeName);
			try
			{
				if (imageCommentPosition < 0)                                            //no comment found
					edit.Insert(line.End, comment + imageLink);
				else if (imageCommentPosition > positionInLine)                           //tried to insert before the comment starts
					edit.Insert(line.End, imageLink);
				else                                                                //somewhere inside the comment
					edit.Insert(location.Position.Position, imageLink);
				edit.Apply();
			}
		...
		public bool HasMatch(string lineText)
		{
			string matchedText;
			return ImageCommentParser.MatchImageTag(_contentTypeName, lineText, out matchedText) >= 0;
		}
		/// <summary>
...
#pragma warning restore 219
			...
			string lineText = line.Extent.GetText();
			string matchedText;
			int matchIndex = ImageCommentParser.MatchImageTag(_contentTypeName, lineText, out matchedText);
			if (matchIndex >= 0)
			{
				...
				else // No existing image, so create new one
				{
					...
					Images.Add(lineNumber, image); //also add in case it fails, so that we don't try again and again
				}
		...
		internal ITextViewLine GetTargetTextViewLine(DragDropInfo dndInfo)
		{
			if (_view.TextViewLines == null)
				return null;

			return _view.TextViewLines.GetTextViewLineContainingYCoordinate(dndInfo.Location.Y);
		}
